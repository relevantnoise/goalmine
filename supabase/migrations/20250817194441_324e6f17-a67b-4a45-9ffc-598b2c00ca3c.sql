-- Update function to use the correct net schema 
...
BEGIN
    ...
    LOOP
        ...
        SELECT public.net.http_post(
            ...
            headers := '{"Content-Type": "application/json", "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRobGN5Y2puendmbmFkbXNwdG9mIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1NTE5MDM3NSwiZXhwIjoyMDcwNzY2Mzc1fQ.4lWJhEJQBOGSGOQ1u7Nwar5xJMq9xA1qLaF2K0k5nRaY"}'::jsonb,
            body := json_build_object(
                'email', goal_record.email,
                'name', split_part(goal_record.email, '@', 1),
                'goal', goal_record.title,
                'message', COALESCE((motivation_response #> '{}')::json->>'message', 'Keep pushing towards your goal!'),
                'microPlan', COALESCE(
                    array_to_string(ARRAY(
                        SELECT json_array_elements_text((motivation_response #> '{}')::json->'microPlan')
                    ), E'\n• '),
                    'Take one step forward today'
                ),
                'challenge', COALESCE((motivation_response #> '{}')::json->>'challenge', 'Challenge yourself today!'),
