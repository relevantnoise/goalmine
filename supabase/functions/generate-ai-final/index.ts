...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    console.log('[GENERATE-AI-FINAL] Starting AI generation with direct database insert...');
    ...
    console.log('[GENERATE-AI-FINAL] Calling ChatGPT...');
...
${JSON.stringify(assessmentData, null, 2)}`
    ...
    if (!openaiResponse.ok) {
      ...
      console.error('[GENERATE-AI-FINAL] OpenAI error:', errorText);
      throw new Error(`OpenAI API error: ${openaiResponse.status} - ${errorText}`);
    }

    const openaiData = await openaiResponse.json();
    console.log('[GENERATE-AI-FINAL] ChatGPT response received successfully!');
    ...
    console.log('[GENERATE-AI-FINAL] AI insights generated:', aiInsights.length, 'insights');

    // Clear existing insights first
    console.log('[GENERATE-AI-FINAL] Clearing existing insights...');
    ...
    if (deleteError) {
      console.error('[GENERATE-AI-FINAL] Failed to clear existing insights:', deleteError);
    } else {
      console.log('[GENERATE-AI-FINAL] Cleared existing insights');
    }

    // Prepare insights for bulk insert - use minimal required fields
    const insightsToSave = aiInsights.map((insight: any, index: number) => ({
    ...
    console.log('[GENERATE-AI-FINAL] Saving insights with bulk insert...');
    const { data: savedInsights, error: insertError } = await supabase
      .from('ai_insights')
      .insert(insightsToSave)
      .select('id, title, content, insight_type, priority');

    if (insertError) {
      console.error('[GENERATE-AI-FINAL] Failed to save insights:', insertError);
      throw new Error(`Failed to save insights: ${insertError.message}`);
    }

    console.log('[GENERATE-AI-FINAL] Successfully saved', savedInsights?.length || 0, 'insights');

    return new Response(JSON.stringify({
      success: true,
      message: `Successfully generated and saved ${savedInsights?.length || 0} AI insights!`,
      data: {
        frameworkId,
        userEmail,
        insightsGenerated: savedInsights?.length || 0,
    ...
    console.error('[GENERATE-AI-FINAL] Error:', error);
    return new Response(JSON.stringify({
      success: false,
      error: error.message,
